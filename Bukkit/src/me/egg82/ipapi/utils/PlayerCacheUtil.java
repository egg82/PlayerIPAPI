...
public class PlayerCacheUtil {
    ...
    public static void addToCache(UUID uuid, String ip, long created, long updated, boolean doInsert) {
        ...
        // Here we really just hope we don't run into any race conditions, because
        // seriously
        ips.remove(ipData);
        ips.add(ipData);
    ...
    public static void addInfo(UUID uuid, String ip) {
        ...
        try (Jedis redis = RedisUtil.getRedis()) {
            if (redis != null) {
                ...
                // Sadly we only update the "updated" param if the value exists so "SETNX" is
                // useless here. We get to round-trip twice, but it's still faster than SQL
