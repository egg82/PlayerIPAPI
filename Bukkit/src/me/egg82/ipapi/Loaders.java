		...
		if (config.getNode("rabbit", "enabled").getBoolean()) {
			ServiceLocator.provideService(
				new RabbitMessageHandler(
					config.getNode("rabbit", "address").getString("127.0.0.1"),
					config.getNode("rabbit", "port").getInt(5672),
					config.getNode("rabbit", "user").getString("guest"),
					config.getNode("rabbit", "pass").getString("guest"),
					pluginName,
