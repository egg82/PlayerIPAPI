...
public class SelectUUIDResultMySQLCommand extends Command {
    // vars
    ...
    private EventHandler<UUIDResultEventArgs> onData = new EventHandler<UUIDResultEventArgs>();

    // constructor
    public SelectUUIDResultMySQLCommand(String ip) {
        super();
    ...
    // public
    public EventHandler<UUIDResultEventArgs> onData() {
        return onData;
    }
    ...
    private void onSQLData(SQLEventArgs e) {
        if (e.getUuid().equals(query)) {
            ...
            UUIDResultEventArgs retVal = null;
            Set<UUIDData> uuids = new HashSet<UUIDData>();
            // Iterate rows
            try (Jedis redis = RedisUtil.getRedis()) {
                ...
                for (Object[] o : e.getData().data) {
                    try {
                        ...
                        if (!ValidationUtil.isValidUuid((String) o[0])) {
                            ...
                            sql.parallelQuery("DELETE FROM `playeripapi` WHERE `uuid`=?;", o[0]);
                ...
                if (e.getData().data.length > 0) {
                    retVal = new UUIDResultEventArgs(ip, uuids);
                }
            }
            ...
            if (retVal != null) {
                ...
                onData.invoke(this, UUIDResultEventArgs.EMPTY);
            }
